...
func newTestTopicProducer(
	t *testing.T, addr, topic string, partitions int32,
	opts ...func(*ProducerConfig),
) *TopicProducer {
	t.Helper()
	...
	p, err := NewTopicProducer(cfg)
	if err != nil {
		t.Fatalf("create topic producer: %v", err)
	}
...
func newTestConsumer(
	t *testing.T, addr, group string,
	opts ...func(*ConsumerConfig),
) *Consumer {
	t.Helper()
